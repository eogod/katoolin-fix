        ...
        def initio1():
            while True:
                ...
                while option0 == "1":
                    ...
                    if repo == "1":
                        cmd1 = os.system(
                            "wget -O - archive.kali.org/archive-key.asc > ~/kali.key")
                        cmd1_2 = os.system('gpg --no-default-keyring --keyring ./kali_keyring.gpg --import kali.key')
                        cmd1_3 = os.system("gpg --no-default-keyring --keyring ./kali_keyring.gpg --export > ./kali.gpg")
                        cmd1_4 = os.system("sudo mv ./kali.gpg /etc/apt/trusted.gpg.d/")
                        cmd1_5 = os.system('''sudo sh -c "echo 'deb https://http.kali.org/kali kali-rolling main non-free contrib' > /etc/apt/sources.list.d/kali.list"''')
                        cmd1_6 = os.system(
                            '''sudo sh -c "echo 'Package: *'>/etc/apt/preferences.d/kali.pref; 
                        echo 'Pin: release a=kali-rolling'>>/etc/apt/preferences.d/kali.pref; echo 'Pin-Priority: 50'>>/etc/apt/preferences.d/kali.pref"''')
                        cmd1_7 = os.system('sudo apt update')
                        cmd2 = os.system(
